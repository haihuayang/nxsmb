...
SET_tests := test-ntlmssp test-security

.PHONY: all target_mkdir host_mkdir
all: $(SET_tests:%=$(TARGET_DIR_out)/tests/%)

#| target_mkdir host_mkdir
...
#t_marshall_size
#misc security ntlmssp
#misc ntlmssp dssetup lsa
idl: target_mkdir host_mkdir $(TARGET_SET_idl:%=$(TARGET_DIR_out)/librpc/idl/%.h) \
	$(TARGET_SET_idl:%= $(TARGET_DIR_out)/librpc/idl/%.ndr.cxx) \
	$(TARGET_SET_idl:%= $(TARGET_DIR_out)/librpc/idl/%.ndr.o)

$(TARGET_SET_idl:%=$(TARGET_DIR_out)/librpc/idl/%.d) : %.d: %.json
	scripts/gen-rpc --depend --outputdir $(dir $@) $<

$(TARGET_SET_idl:%=$(TARGET_DIR_out)/librpc/idl/%.d) : scripts/gen-rpc

$(TARGET_SET_idl:%=$(TARGET_DIR_out)/librpc/idl/%.d):
...
SET_OBJ_SRC_nxsmbd := \
	$(SET_src_nxsmbd:%=$(TARGET_DIR_out)/src/%.o)
...
SET_OBJ_libheimdal := \
		$(TARGET_SET_asn1:%=$(TARGET_DIR_out)/samba/source4/heimdal/%_asn1.o) \
		$(SET_SRC_heimdal:%=$(TARGET_DIR_out)/samba/source4/heimdal/%.o) \
		$(SET_SRC_com_err:%=$(TARGET_DIR_out)/samba/source4/heimdal/lib/com_err/%.o) \
		$(SET_SRC_krb5:%=$(TARGET_DIR_out)/samba/source4/heimdal/lib/krb5/%.o) \
		$(SET_SRC_roken:%=$(TARGET_DIR_out)/samba/source4/heimdal/lib/roken/%.o) \
		$(SET_SRC_hx509:%=$(TARGET_DIR_out)/samba/source4/heimdal/lib/hx509/%.o) \
		$(SET_PROTO_der:%=$(TARGET_DIR_out)/samba/source4/heimdal/%.o) \
		$(TARGET_SET_et:%=$(TARGET_DIR_out)/samba/source4/heimdal/%.o) \
		$(SET_GEN_wind:%=$(TARGET_DIR_out)/samba/source4/heimdal/lib/wind/%.o) \
		$(TARGET_DIR_out)/samba/source4/heimdal/lib/roken/resolve.o \
		$(TARGET_DIR_out)/samba/source4/heimdal/lib/asn1/timegm.o \
		$(TARGET_DIR_out)/samba/source4/heimdal/lib/asn1/extra.o \
		$(TARGET_DIR_out)/samba/source4/heimdal/lib/gssapi/spnego/compat.o \
		$(TARGET_DIR_out)/samba/lib/replace/replace.o \
		$(TARGET_DIR_out)/samba/source4/heimdal_build/gssapi-glue.o \
		$(TARGET_DIR_out)/samba/source4/heimdal_build/replace.o \
...
$(SET_tests:%=$(TARGET_DIR_out)/tests/%) : %: %.o $(SET_libs:%=$(TARGET_DIR_out)/lib%.a) | $(TARGET_DIR_out)/tests
	$(CXX) -g -o $@ $^ -lpthread -lresolv -ldl

$(TARGET_DIR_out)/bin/nxsmbd: $(SET_OBJ_SRC_nxsmbd) $(SET_OBJ_samba)
	$(CXX) -g -o $@ $^ -lpthread -lresolv -ldl

$(TARGET_DIR_out)/libnxsmb.a: $(SET_OBJ_libnxsmb:%=$(TARGET_DIR_out)/%.o) $(TARGET_SET_idl:%=$(TARGET_DIR_out)/librpc/idl/%.ndr.o)
	ar rcs $@ $^

$(TARGET_DIR_out)/librpc/idl/%.ndr.o: $(TARGET_DIR_out)/librpc/idl/%.ndr.cxx | $(TARGET_SET_idl:%=$(TARGET_DIR_out)/librpc/idl/%.json)
...
.SECONDARY:
$(TARGET_SET_idl:%=$(TARGET_DIR_out)/librpc/idl/%.json): $(TARGET_DIR_out)/librpc/idl/%.json: samba/librpc/idl/%.idl | $(TARGET_DIR_out)/librpc/idl
	CPP=/usr/bin/cpp CC=/usr/bin/gcc /usr/bin/perl samba/pidl/pidl --quiet --dump-json $< > $@

$(TARGET_DIR_out)/lib/%.o: lib/%.cxx | $(TARGET_DIR_out)/lib/librpc
	$(CXX) -c $(TARGET_CXXFLAGS) $(TARGET_CFLAGS_EXTRA) -o $@ $<

$(TARGET_DIR_out)/src/%.o: src/%.cxx
	$(CXX) -c $(TARGET_CXXFLAGS) $(TARGET_CFLAGS_EXTRA) -o $@ $<

$(TARGET_DIR_out)/tests/%.o: tests/%.cxx | $(TARGET_DIR_out)/tests
	$(CXX) -c $(TARGET_CXXFLAGS) $(TARGET_CFLAGS_EXTRA) -o $@ $<

$(SET_tests:%=$(TARGET_DIR_out)/tests/%.o): $(TARGET_SET_idl:%=$(TARGET_DIR_out)/librpc/idl/%.h)

$(TARGET_DIR_out)/samba/source4/%.o: $(TARGET_DIR_out)/samba/source4/%.c
	$(CC) -c $(TARGET_CFLAGS) $(TARGET_CFLAGS_EXTRA) -o $@ $<

$(TARGET_DIR_out)/samba/source4/%.o: samba/source4/%.c
	$(CC) -c $(TARGET_CFLAGS) $(TARGET_CFLAGS_EXTRA) \
		-DBINDIR=\"/usr/bin\" -DSBINDIR=\"/usr/sbin\" \
		-DLIBDIR=\"/usr/lib\" -DLIBEXECDIR=\"/usr/libexec\" \
		-o $@ $<

$(TARGET_DIR_out)/samba/lib/%.o: samba/lib/%.c
	$(CC) -c $(TARGET_CFLAGS) $(TARGET_CFLAGS_EXTRA) -o $@ $<
...
#$(SET_SRC_heimdal:%=$(TARGET_DIR_out)/samba/source4/heimdal/%.o)
$(TMP_SET_OBJ_SRC_nxsmbd) $(TMP_SET_tests:%=$(TARGET_DIR_out)/tests/%.o): \
...
$(TARGET_DIR_out)/samba/source4/heimdal/lib/wind/bidi_table.h: samba/source4/heimdal/lib/wind/rfc3454.txt
	python samba/source4/heimdal/lib/wind/gen-bidi.py $< $(dir $@)

$(TARGET_DIR_out)/samba/source4/heimdal/lib/wind/map_table.h: samba/source4/heimdal/lib/wind/rfc3454.txt
	python samba/source4/heimdal/lib/wind/gen-map.py $< $(dir $@)

$(TARGET_DIR_out)/samba/source4/heimdal/lib/wind/errorlist_table.h: samba/source4/heimdal/lib/wind/rfc3454.txt
	python samba/source4/heimdal/lib/wind/gen-errorlist.py $< $(dir $@)

$(TARGET_DIR_out)/samba/source4/heimdal/lib/wind/normalize_table.h: samba/source4/heimdal/lib/wind/UnicodeData.txt samba/source4/heimdal/lib/wind/CompositionExclusions-3.2.0.txt
	python samba/source4/heimdal/lib/wind/gen-normalize.py $^ $(dir $@)

$(TARGET_DIR_out)/samba/source4/heimdal/lib/wind/combining_table.h: samba/source4/heimdal/lib/wind/UnicodeData.txt
...
HOST_SET_dir := bin samba/include samba/lib/replace samba/source4/heimdal_build $(HOST_SET_heimdal:%=samba/source4/heimdal/%) \
