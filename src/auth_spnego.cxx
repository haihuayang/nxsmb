...
static inline void spnego_wrap(NTSTATUS status, MechType *mt, void *mic_data,
		uint32_t mic_length, const std::vector<uint8_t> &subout,
		std::vector<uint8_t> &out)
{
	auto negResult = NT_STATUS_IS_OK(status) ?
		NegTokenResp::accept_completed : NegTokenResp::accept_incomplete;
	x_spnego_wrap_resp(&negResult, mt, mic_data, mic_length,
			subout, out);
}
		...
		std::vector<uint8_t>& subout, std::vector<uint8_t> &out)
{
	if (NT_STATUS_IS_OK(status)) {
		...
		spnego_wrap(status, nullptr, mech_list_mic.data(),
				x_convert_assert<uint32_t>(mech_list_mic.size()),
				subout, out);
		spnego->state_position = x_auth_spnego_t::DONE;
...
static int x_auth_spnego_create_negTokenInit(std::vector<uint8_t> &out)
{
	...
	*spnego_token.u.negTokenInit.negHints->hintName = strdup("not_defined_in_RFC4178@please_ignore");
	...
	x_spnego_wrap_gssapi(spnego_data, out);
		...
		std::shared_ptr<x_auth_info_t> &auth_info)
{
	...
	if (spnego->state_position == x_auth_spnego_t::SERVER_START) {
		if (in_len > 0) {
			NegotiationToken nt;
			int err = x_spnego_decode_token(nt, in_buf, in_len);
			if (err) {
				return NT_STATUS_INVALID_PARAMETER;
			}
