		...
		x_smbd_requ_t *smbd_requ)
{
	...
	x_smb2_out_flush_t *out_flush = (x_smb2_out_flush_t *)(out_hdr + SMB2_HDR_BODY);
	out_flush->struct_size = X_H2LE16(sizeof(x_smb2_out_flush_t));
	out_flush->reserved0 = 0;

	x_smb2_reply(smbd_conn, smbd_requ, bufref, bufref, NT_STATUS_OK, 
			SMB2_HDR_BODY + sizeof(x_smb2_out_flush_t));
}

NTSTATUS x_smb2_process_flush(x_smbd_conn_t *smbd_conn, x_smbd_requ_t *smbd_requ)
{
	if (smbd_requ->in_requ_len < SMB2_HDR_BODY + sizeof(x_smb2_in_flush_t)) {
		RETURN_OP_STATUS(smbd_requ, NT_STATUS_INVALID_PARAMETER);
	}

	const uint8_t *in_hdr = smbd_requ->get_in_data();
	const x_smb2_in_flush_t *in_flush = (const x_smb2_in_flush_t *)(in_hdr + SMB2_HDR_BODY);
