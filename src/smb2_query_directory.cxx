...
{
	if (smbd_requ->in_requ_len < SMB2_HDR_BODY + sizeof(x_smb2_in_find_t)) {
	...
	if (!decode_in_find(*state, in_hdr, smbd_requ->in_requ_len)) {
		RETURN_OP_STATUS(smbd_requ, NT_STATUS_INVALID_PARAMETER);
	}
	...
	NTSTATUS status = x_smbd_open_op_find(smbd_conn, smbd_requ, state);
	if (NT_STATUS_IS_OK(status)) {
		x_smb2_reply_find(smbd_conn, smbd_requ, *state);
		return status;
	}
