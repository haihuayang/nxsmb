...
static void x_smb2_reply_read(x_smbd_requ_t *smbd_requ,
		x_smbd_requ_state_read_t &state)
{
...
NTSTATUS x_smbd_requ_read_t::done_smb2(x_smbd_conn_t *smbd_conn, NTSTATUS status)
{
	...
	if (status.ok()) {
		if (state.out_buf_length < state.in_minimum_count) {
			...
			x_smb2_reply_read(this, this->state);
		}
	}
	return status;
}
		...
		bool encrypted)
{
	...
	if (in_buf.length < sizeof(x_smb2_header_t) + sizeof(x_smb2_read_requ_t)) {
		X_SMBD_SMB2_RETURN_STATUS(in_smb2_hdr, NT_STATUS_INVALID_PARAMETER);
	}

	auto in_body = (const x_smb2_read_requ_t *)(in_smb2_hdr + 1);
	auto in_flags = X_LE2H8(in_body->flags);
	auto in_length = X_LE2H32(in_body->length);
	auto in_offset = X_LE2H64(in_body->offset);
	auto in_file_id_persistent = X_LE2H64(in_body->file_id_persistent);
	auto in_file_id_volatile = X_LE2H64(in_body->file_id_volatile);
	auto in_minimum_count = X_LE2H32(in_body->minimum_count);

	if (in_length > x_smbd_conn_get_negprot(smbd_conn).max_read_size) {
		X_SMBD_SMB2_RETURN_STATUS(in_smb2_hdr, NT_STATUS_INVALID_PARAMETER);
	}

	if (!valid_io_range(in_offset, in_length)) {
		X_SMBD_SMB2_RETURN_STATUS(in_smb2_hdr, NT_STATUS_INVALID_PARAMETER);
	}
	...
	state.in_minimum_count = in_minimum_count;
