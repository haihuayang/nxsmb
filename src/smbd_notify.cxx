...
struct smd_notify_evt_t
{
	static void func(void *ctx_conn, x_fdevt_user_t *fdevt_user)
	{
		...
		auto state = nxfsd_requ->release_state<x_smbd_requ_state_notify_t>();
		X_ASSERT(state->out_notify_changes.empty());
		std::swap(state->out_notify_changes, evt->notify_changes);
		state->async_done(ctx_conn, nxfsd_requ, NT_STATUS_OK);
		delete evt;
	}

	smd_notify_evt_t(x_nxfsd_requ_t *requ,
			std::vector<std::pair<uint32_t, std::u16string>> &&changes)
		: base(func), nxfsd_requ(requ), notify_changes(changes)
	{
	}
		...
		long open_priv_data)
{
	...
	for (curr_open = open_list.get_front(); curr_open; curr_open = open_list.next(curr_open)) {
		...
		for (nxfsd_requ = curr_open->pending_requ_list.get_front();
			...
			if (nxfsd_requ->get_requ_state<x_smbd_requ_state_notify_t>()) {
				break;
			}
		}
		if (!nxfsd_requ) {
		...
		X_NXFSD_REQU_POST_USER(nxfsd_requ, 
				new smd_notify_evt_t(nxfsd_requ,
