...
{
	X_SMBD_COUNTER_INC_CREATE(smbd_object, 1);
	if (parent_object) {
...
{
	X_SMBD_COUNTER_INC_DELETE(smbd_object, 1);
	if (parent_object) {
		{
			auto lock = std::lock_guard(parent_object->mutex);
...
{
	X_SMBD_COUNTER_INC_CREATE(smbd_stream, 1);
}

x_smbd_stream_t::~x_smbd_stream_t()
{
	X_SMBD_COUNTER_INC_DELETE(smbd_stream, 1);
}
